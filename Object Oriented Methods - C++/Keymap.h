...
public:
	...
	int enlargeMap() {
		...
		capacity++;								// increment array size
		K* tempKeyA = new int[capacity];		// create new bigger array.
		V* tempValA = new int[capacity];
		for (int i = 0; i < capacity; i++) {
			K key = keyArray[i];			// copy values to new array.
			V val = valArray[i];
		...
		delete[] keyArray;						// free old array memory.
		delete[] valArray;					// leak time (dancingpug.gif)
